...
int main(int argc, char *argv[]) {
    ...
    if (id == 0) {
        ...
        if (strlen(search_word) == 0) {
            ...
            fprintf(stderr, "Uso: %s -k <clave> -r <radio> -s <palabra> [-f <archivo>]\n", argv[0]);
            MPI_Abort(comm, 1);
        }
    ...
    if (id == 0) {
        ...
        printf("=== DES BRUTE FORCE - BÚSQUEDA RADIAL MPI ===\n");
    ...
    for (long radius = id; radius <= search_radius && found == 0; radius += N) {
        ...
        // Generar claves en esta capa (bidireccional)
        long keys_in_layer[2];
        int valid_keys = 0;

        // Lado negativo (centro - radio)
        long key_minus = known_key - radius;
        if (key_minus >= 0 && key_minus < (1L << 56)) {
            keys_in_layer[valid_keys++] = key_minus;
        }

        // Lado positivo (centro + radio), evitar duplicado en radio=0
        if (radius > 0) {
        ...
        for (int k = 0; k < valid_keys && found == 0; k++) {
            ...
            if (tryKey(key, buffer, ciphlen, local_temp_buffer, search_word)) {
                found = key;
                printf("\n>>> Proceso %d ENCONTRÓ LA CLAVE: %ld (radio: %ld) <<<\n", 
                       id, key, radius);
        ...
        // Reporte de progreso (solo proceso 0, cada 2 segundos aproximadamente)
    ...
    if (id == 0) {
        ...
        if (found != 0) {
            ...
            // Calcular speedup
            double sequential_time = total_keys_tested / (total_keys_tested / total_time);
            double speedup = sequential_time / total_time;
            printf("Speedup estimado con %d procesos: %.2fx\n", N, speedup);
            printf("Eficiencia: %.1f%%\n", (speedup / N) * 100);
            
            // Estadísticas de búsqueda radial
            double radius_explored = (double)labs(found - known_key);
            double percent_explored = (radius_explored / search_radius) * 100;
            printf("\nEstadísticas de búsqueda radial:\n");
            printf("- Radio máximo explorado: ~%.0f\n", radius_explored);
            printf("- Porcentaje del radio total: %.2f%%\n", percent_explored);
            printf("- Reducción de espacio: %.2f%%\n", 100 - percent_explored);
            ...
            printf("Velocidad: %.0f claves/segundo\n", 
                   total_time > 0 ? total_keys_tested / total_time : 0.0);
            printf("\nSugerencia: Incremente el radio de búsqueda con -r\n");
        }
    }
